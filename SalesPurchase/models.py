...
class PurchaseQuotation(models.Model):
    ...
    quotation_id = models.AutoField(primary_key=True)
    quotation_number = models.CharField(max_length=50, unique=True)
    purchase_rfq = models.ForeignKey(PurchaseRFQ, on_delete=models.CASCADE, null=True, blank=True, related_name='rfq_quotations')
    supplier = models.ForeignKey(Supplier, on_delete=models.CASCADE, related_name='supplier_quotations')
    quotation_date = models.DateField()
    expiry_date = models.DateField()
    currency = models.CharField(max_length=3, choices=CURRENCY_CHOICES, default='USD')
    status = models.CharField(max_length=20, choices=QUOTATION_STATUS, default='draft')
    ...
    tax_amount = models.DecimalField(max_digits=12, decimal_places=2, default=0)
    total_amount = models.DecimalField(max_digits=12, decimal_places=2, default=0)
    created_by = models.ForeignKey(CustomUser, on_delete=models.SET_NULL, null=True, related_name='purchase_quotations_created')
    created_date = models.DateTimeField(auto_now_add=True)
    last_modified = models.DateTimeField(auto_now=True)
    
    def save(self, *args, **kwargs):
        if not self.quotation_number:
            last_quote = PurchaseQuotation.objects.order_by('-created_date').first()
            if last_quote:
                last_number = int(last_quote.quotation_number.split('-')[-1])
                self.quotation_number = f"PQ-{str(last_number + 1).zfill(5)}"
            else:
                self.quotation_number = "PQ-00001"
        super().save(*args, **kwargs)
    ...
    def get_absolute_url(self):
        return reverse('purchase_quotation_detail', kwargs={'pk': self.pk})
    
    def can_create_order(self):
        return self.status in ['received', 'accepted'] and self.items.exists()
...
class PurchaseQuotationItem(models.Model):
    ...
    rfq_item = models.ForeignKey(PurchaseRFQItem, on_delete=models.SET_NULL, null=True, blank=True)
    component = models.ForeignKey(Component, on_delete=models.CASCADE)
    description = models.TextField()
    quantity = models.PositiveIntegerField(validators=[MinValueValidator(1)])
    unit_price = models.DecimalField(max_digits=12, decimal_places=2, validators=[MinValueValidator(Decimal('0.01'))])
    tax_rate = models.DecimalField(max_digits=5, decimal_places=2, default=0)
    line_total = models.DecimalField(max_digits=12, decimal_places=2, default=0)
    lead_time = models.PositiveIntegerField(help_text="Lead time in days", default=0)
    notes = models.TextField(blank=True, null=True)
    ...
    def __str__(self):
        return f"{self.quotation.quotation_number} - {self.component.name}"
